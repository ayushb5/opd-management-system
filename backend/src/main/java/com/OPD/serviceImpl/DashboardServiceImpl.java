...
public class DashboardServiceImpl implements DashboardService {
	...
	public DashboardResponse getAdminDashboard() {
		...
		long pendingVisits = visitRepository
				.countByStatusIn(List.of(Visit.Status.WAITING, Visit.Status.IN_CONSULTATION));
		...
		response.setWeeklyVisits(visitRepository.findDailyVisitCounts(sevenDaysAgo));

		response.setMonthlyVisits(visitRepository.findMonthlyVisitCounts());

		response.setPaymentBreakdown(billRepository.findPaymentBreakdown());
	...
	public DashboardResponse getDoctorDashboard(Integer doctorId) {
		...
		long pendingVisits = visitRepository.countByDoctor_IdAndVisitDateAndStatusIn(doctorId, LocalDate.now(),
				List.of(Visit.Status.WAITING, Visit.Status.IN_CONSULTATION));
		response.setPendingVisits(pendingVisits);
		...
		LocalDate sevenDaysAgo = LocalDate.now().minusDays(6);
		response.setWeeklyVisits(visitRepository.findDailyVisitCountsByDoctor(doctorId, sevenDaysAgo));

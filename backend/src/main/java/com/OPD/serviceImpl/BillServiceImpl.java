...
public class BillServiceImpl implements BillService {
	...
	public Bill save(Bill bill) {
		...
		if (bill.getConcession().add(bill.getPaidAmount()).compareTo(bill.getTotalAmount()) > 0) {
			throw new BadRequestException("Concession + Paid Amount cannot exceed Total Amount");
		}

		Bill existingBill = repository.findByVisit_Id(bill.getVisit().getId());

		if (existingBill != null && (bill.getId() == null || !existingBill.getId().equals(bill.getId()))) {
			throw new BadRequestException("Bill already exists for visit id: " + bill.getVisit().getId());
		}
	...
	public Page<Bill> getAllBills(int page, int size, String search) {
		...
		return repository.findByVisit_Patient_PatientNameContainingIgnoreCase(search, pageable);
	}
	...
	public Bill getBillByVisitId(Integer visitId) {
		...
		if (bill == null) {
			throw new ResourceNotFoundException("Bill not found for visit id: " + visitId);
		}
